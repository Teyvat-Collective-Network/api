...
export async function validateInvite(raw: string, guild: string): Promise<string> {
    ...
    const invite: { code: string; id: string; vanity: boolean; target: boolean } = await req.json();
    if (invite.id !== guild) throw new APIError(400, codes.INVALID_INVITE, "The invite does not point to the correct guild.");
